...
#define VIDEO_H265_MAJOR			6			//H265主码流
#define VIDEO_H265_MINOR			7			//H265次码流
...
{
	uint32_t lChannel;    /**< 通道号，从0开始 */
	uint32_t lStreamMode; /**< 码流类型，0-主码流，1-子码流 */
 ...
 * - 按位表示，为1表示有效，0表示无效
 * - 从低位到高位与栅格列数从左到右一一对应
 * - PAL制时为18行22列的栅格，NTSC制下为15行22列的栅格，即对应数组的前15个值
 */
typedef struct
{
	unsigned int dwMotionScope[18];	/**< 18/15行22列按位表示，为1表示有效为0表示无效*/
}MD_Scope_Info;
...
* @note 无
*/
int32_t NET_IPC_Login(const char         *sDevIP,
...
* @note 无
*/
BOOL NET_IPC_Logout(int32_t lLoginID);
...
* @note 无
*/
BOOL NET_IPC_StopRealData(int32_t lRealHandle,int iStreamID);
...
* @note 无
*/
BOOL NET_IPC_CloseRealDataConnect(int32_t lRealHandle,int iStreamID,int iSockfd);
...
*/
BOOL  NET_IPC_PTZControl(int32_t          lLoginID,
...
* @param[in] cRes 		 保留
*/
BOOL NET_IPC_TransparantSerialControl(int32_t lLoginID,
	int nChannel,
 ...
 * - 获取错误码调用NET_IPC_GetLastError
 */
BOOL NET_IPC_AlarmStartListen(int32_t lLoginID, fAlarmMsgCallBack DataCallback);
 ...
 */
typedef void (*fRealDataCallBack)(int32_t lRealHandle, int iDataType, BYTE cRrameType, BYTE cFrameRate, BYTE *pFrameBuf, unsigned int iFrameSize, uint32_t lTimeStamp, void *pUser);
 ...
 * @return 返回实时预览监听句柄
 */
int32_t NET_IPC_StartRealPlay(int32_t lLoginID,short sPort,S_REALPLAY_INFO  *pRealDataInfo,fRealDataCallBack fRealData,void *pUserData);
 ...
 * - 获取错误码调用NET_IPC_GetLastError
 */
BOOL NET_IPC_StopRealPlay(int32_t lRealHandle);
 ...
 * - 获取错误码调用NET_IPC_GetLastError
 */
BOOL NET_IPC_VoiceFrameInput(int32_t lLoginID,short sPort,S_REALPLAY_INFO  *pRealDataInfo,BYTE *pFrameBuf, unsigned int iFrameSize,void *pUserData);
 ...
 * - 获取错误码调用NET_IPC_GetLastError
 */
BOOL NET_IPC_StopVoiceInput(int32_t lLoginID);

/**
 * @brief - NET_IPC_GetMDScope
 * - 获取移动侦测区域信息
 * @param[in]  lLoginID 用户ID 号，NET_IPC_Login的返回值
 * @param[in]  sPort 通信端口，默认90
 * @param[out] format 视频制式 0-PAL 1-NTSC
 * @param[out] scope 区域信息
 * @return 返回结果如下:
 * - 成功: true
 * - 失败: false
 * - 获取错误码调用NET_IPC_GetLastError
 */
BOOL NET_IPC_GetMDScope(int32_t lLoginID,short sPort,int *format,MD_Scope_Info *scope);

/**
 * @brief - NET_IPC_SetMDScope
 * - 设置移动侦测区域信息
 * @param[in]  lLoginID 用户ID 号，NET_IPC_Login的返回值
 * @param[in]  sPort 通信端口，默认90
 * @param[in]  scope 区域信息
 ...
 * - 获取错误码调用NET_IPC_GetLastError
 */
BOOL NET_IPC_SetMDScope(int32_t lLoginID,short sPort,MD_Scope_Info *scope);
